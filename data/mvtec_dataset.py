...
# DATASETS_PATH = '/home/liuj35/complete/mvtec3d_new/'
...
class MVTec3DTrain(MVTec3D):
    ...
    def __getitem__(self, idx):
        ...
        return (0, pcd_np_normalize, resized_organized_pc, resized_depth_map_3channel, nonzero_indices), label
    
class MVTec3DValid(MVTec3D):
    def __init__(self, class_name, img_size):
        super().__init__(split="validation", class_name=class_name, img_size=img_size)
...
def get_data_loader(split, class_name, img_size, batch_size=10):
    if split in ['train']:
        ...
        data_loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, num_workers=6, drop_last=True,
                             pin_memory=True)
    elif split in ['validation']:
        dataset = MVTec3DValid(class_name=class_name, img_size=img_size)
        data_loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=False, num_workers=6, drop_last=False,
                             pin_memory=True)
    elif split in ['test']:
